function [ reswalking ] = walkinginkdancing( trj, knntrj, p0, q, m, tmax )
...
%   knntrj: 
%   p0: start position
%   q: [qjump,qdance] with probability qjump jump to k nearest neighbor, with
%     probability 1-qjump jump to the position of next 
%     (or previous, decided by current direction) frame;
%     with probability qdance change direction, or change direction reaching the end
...
for i=2:tmax
    qjumpsample = rand(m,1);
    %these walkers jump to nearest neighbor
    walkernn = find(qjumpsample<qjump);
    %other walkers walk along this trajectory
    walkernormal = find(~(qjumpsample<qjump));
    ...
    %sample dancing walkers and change their directions
    qdancingsample = rand(m,1);
    %these walkers jump to nearest neighbor
    walkerdancing = find(qdancingsample<qdance);
    directionthis(walkerdancing,1) = -directionthis(walkerdancing,1);

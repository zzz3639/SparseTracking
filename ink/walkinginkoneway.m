function [ reswalking ] = walkinginkoneway( trj, knntrj, p0, q, m, tmax )
% random walk(monte carlo walk) for trajectory set trj start from position p0
%   Do not allow monte carlo particles walk backward
% input:
...
% in this function, monte carlo particles are forced to jump to nearest
% neighbor if it reaches the end of a trajectory. directionthis(i)==-1 means
% monte carlo particle i reaches the end.
...
for i=2:tmax
    ...
    logicjump = (qsample<q)|(directionthis==-1);
    walkernn = find(logicjump);
    %other walkers walk along this trajectory
    walkernormal = find(~logicjump);
    ...
    [jumpsituI,~,jumpsituS] = find(knntrj(idxthis(walkernn,1),:).*jumpsample);
    [~,v] = sort(jumpsituI);
    idxnext(walkernn,1) = jumpsituS(v);
    
    %define walking directions in the next step
    directionthis(walkernn,1) = ones(length(walkernn),1);

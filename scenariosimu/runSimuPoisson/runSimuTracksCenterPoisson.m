...
Centerinfo = [center,0.05]; % center location, centripetal movement speed
% generate data with ground truth
[Movies, Trj] = SimuTracksCenterPoisson(Smol, S, N/svl, svl-1, Imean, Dint, alpha, Dmu, Centerinfo, ones(Smol(1),Smol(2)), zeros(0,3), Nframes, 1, 1, no*S(1)*S(2), sig, 0);
...
[Movies, Trj] = SimuTracksCenterPoisson(Smol, S, N/svl, svl-1, Imean, Dint, alpha, Dmu, Centerinfo, ones(Smol(1),Smol(2)), zeros(0,3), Nframestunning, 1, 1, no*S(1)*S(2), sig, 0);
...
save(['ScenarioOrbitD03F5000Poisson.mat']);

...
[Movies, Trj] = SimuTracksDecelerate(Smol, S, N/svl, svl-1, Imean, Dint, alpha, Dmu, ones(Smol(1),Smol(2)), zeros(0,3), Nframes, 1, 1, noscale*S(1)*S(2), sig, 0);
...
[Moviestunning, Trjtunning] = SimuTracksDecelerate(Smol, S, N/svl, svl-1, Imean, Dint, alpha, Dmu, ones(Smol(1),Smol(2)), zeros(0,3), Nframestunning, 1, 1, noscale*S(1)*S(2), sig, 0);
...
trjtruetunning.no = 50*S(1)*S(2)*ones(Tmax,1);
clear trj trjlong Moviestunning Trjtunning Movies Trj
save(['ScenarioDecelerateD03.mat']);
